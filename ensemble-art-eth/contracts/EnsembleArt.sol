...
import "@mentaport/solidity-contracts/contracts/main/MentaportMint.sol";
...
contract EnsembleArt is MentaportMint {
        ...
        returns (uint256)
    {
        ...
        require(false, "Use mintWithLocationProps.");
        return 0;
    }
        ...
        )
    {
        ...
        require(
            _activeLocationRules[_mintRequest.locationRuleId],
            "rule provided not active"
        );
    ...
    function getTokenLocationRule(
        uint256 tokenId
    ) public view returns (uint256 locationRuleId, uint256 locationRuleIndex) {
        return _getLocationFromTokenId(tokenId);
    }

    function getLocationRuleSize(
        uint256 locationRuleId
    ) public view returns (uint256) {
        return _usedLocationRules[locationRuleId].length;
    }

    function getLocationPropByIndex(
        uint256 locationRuleId,
        uint256 locationRuleIndex
    ) public view returns (LocationProp memory locationProp) {
        locationProp = _usedLocationRules[locationRuleId][locationRuleIndex];
        return locationProp;
    }

    function getLocationPropByTokenId(
        uint256 tokenId
    ) public view returns (LocationProp memory locationProp) {
        (
            uint256 locationRuleId,
            uint256 locationRuleIndex
        ) = _getLocationFromTokenId(tokenId);
        locationProp = _usedLocationRules[locationRuleId][locationRuleIndex];
        return locationProp;
    }

    function getLocationPropValuesByTokenId(
        uint256 tokenId
    ) public view returns (string memory colorName, string memory shapeName) {
        (
            uint256 locationRuleId,
            uint256 locationRuleIndex
        ) = _getLocationFromTokenId(tokenId);
        LocationProp memory locationProp = _usedLocationRules[locationRuleId][
            locationRuleIndex
        ];
    ...
    function _getLocationFromTokenId(
        uint256 tokenId
    )
        internal
        view
        returns (uint256 _locationRuleId, uint256 _locationRuleIndex)
    {
    ...
    function _mint(
        ...
        require(
            locationProp.colorSelection <
                _colorsRules[_mintRequest.locationRuleId].length,
            "Color index out of bound"
        );
        require(
            locationProp.shapeSelection <
                _shapesRules[_mintRequest.locationRuleId].length,
            "Shape index out of bound"
        );
